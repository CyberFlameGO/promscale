...
func (t tag) Before(item queable) bool {
...
func (t tag) Queries() ([]string, [][]interface{}) {
	return []string{
			fmt.Sprintf(insertTagKeySQL, schema.TracePublic, schema.TracePublic),
			fmt.Sprintf(insertTagSQL, schema.TracePublic, schema.TracePublic),
		},
		[][]interface{}{
			[]interface{}{t.key, t.typ},
			[]interface{}{t.key, t.value, t.typ},
		}
}

func (t tag) Result(r pgx.BatchResults) (interface{}, error) {
	var id tagIDs
...
func newTagBatch(cache *clockcache.Cache) tagBatch {
	return tagBatch{
		b: newBatcher(cache),
	}
}
